...
<template>
  <div class="container-fluid d-flex flex-column h-100">
    <div class="row flex-fill">
      <div class="col-12">
        <h1>
          Scan Docs for broken example links
        </h1>
      </div>
    </div>
...
export default {
		...
		tableData2: function () {
			...
			if ( this.content2 ) {
				...
				return Object.keys( this.content2 ).reduce( ( all, file ) => {
					...
					this.content2[ file ].forEach( example => all.push( { page: file, example: example } ) );
		...
		tableDiff: function () {
			...
			if ( this.diffAll ) {
				...
				return Object.keys( this.diffAll ).reduce( ( all, page ) => {
					...
					this.diffAll[ page ].forEach( example => {

						all.push( { page, example } );
	...
	methods: {
		...
		_fetchFilesOfRevision( rev ) {
			...
			return fetch( `${API_URL}/checkDocsExamples/showFile/${rev}` )
				.then( res => res.json() )
		...
		pullRevisions() {
			...
			return fetch( `${API_URL}/checkDocsExamples/revisions` )
				.then( res => res.json() )
				.then( revs => {
