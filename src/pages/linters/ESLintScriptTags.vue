    ...
    >
      ...
      <div class="col h-100">
        <div
          ...
          style="height: 95%!important; overflow: auto"
        >
          <template
            v-if="tableData"
          >
...
export default {
	...
	data: function () {
		...
		return {
			...
			tableHeaders: [ {
				...
				name: 'severity', label: 'Type', sortable: true
			}, {
	...
	watch: {
		...
		selectedFilename: function ( /* file */ ) {
			...
			if ( this.filename !== this.selectedFilename ) {
				...
				this.$router.push( { name: 'linterEslintScriptTags', params, query } );
	...
	methods: {
		...
		_fetchFilesOfRevision( rev ) {
			...
			return fetch( `${API_URL}/linterEslintScriptTags/showFile/${rev}` )
				.then( res => res.json() )
